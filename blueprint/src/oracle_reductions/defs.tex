...
We have a lattice of security notions, with knowledge and round-by-round being two strengthenings
of soundness.  The formalized lattice (in \texttt{Security/ImplicationsCore.lean}) records two
honesty caveats discovered during formalization: (a) the current Lean definition of straightline
knowledge soundness is \emph{trivially satisfiable} (the extractor is \texttt{OptionT}-valued and
extraction failure silently erases the bad event --- machine-checked as
\texttt{Verifier.knowledgeSoundness\_vacuous}), so the genuine content of the
RBR-KS~$\Rightarrow$~KS implication lives in the variant exhibiting the real fold-of-%
\texttt{extractMid} extractor; and (b) the unconditional RBR~$\Rightarrow$~plain implications are
\emph{false} for state-mutating shared-oracle implementations, so the formalized statements are
conditional on a marginal-domination side condition dischargeable for honest implementations.

\begin{theorem}[Knowledge Soundness Implies Soundness (open; false as literally stated)]
    ...
    \emph{Status: not formalizable as stated} — under the current definitions the hypothesis is
    vacuous (see above), the knowledge game tests the prover's own output witness while soundness
    tests language membership, and the witness types are quantified differently.  The proposed
    honest variant (never-failing extractor + output-relation side condition) is documented in
    \texttt{Security/ImplicationsCore.lean}.
...
\begin{theorem}[RBR Soundness Implies Soundness]
    ...
    $\sum_i \epsilon(i)$, where the sum is over all challenge rounds $i$ --- \emph{conditional on
    the marginal-domination side condition} (the unconditional statement is false for
    state-mutating shared-oracle implementations; the side condition is discharged generically
    for state-preserving, never-failing, value-blind implementations).
    \lean{Verifier.rbrSoundness_implies_soundness_of_marginal}
...
\begin{theorem}[RBR Knowledge Soundness Implies RBR Soundness]
    ...
    soundness with the same error function $\epsilon$ (for the languages of the relations;
    nonemptiness of the witness types is required to adapt arbitrary-witness provers).
...
\begin{theorem}[RBR Knowledge Soundness Implies Knowledge Soundness]
    ...
    with error $\sum_i \epsilon(i)$, where the sum is over all challenge rounds $i$.  The literal
    statement is proven (but vacuous --- see above); the genuine variant, exhibiting the
    straightline extractor obtained by folding the round-by-round extractor along the realized
    transcript and proving the $\sum_i \epsilon(i)$ bound for \emph{that} extractor, is
    \texttt{rbrKnowledgeSoundness\_implies\_knowledgeSoundness\_genuine}, conditional on two named
    transport residuals (fresh-init seam and per-round marginal transport).
...
\begin{definition}[State-Restoration Knowledge Soundness]
    ...
    Equivalently, for every state-restoration prover, the probability that the prover outputs a
    valid output witness and induces an accepting verifier output, while the extractor fails to
    produce any valid input witness, is bounded by the state-restoration knowledge-soundness error.
    The extractor is partial in the Lean formalization, so extractor failure counts as a bad event
    on accepting executions.

    \lean{Verifier.StateRestoration.knowledgeSoundness}
\end{definition}
