...
\begin{remark}[Current STIR status]\label{rem:stir_status}
    The blueprint is complete as a dependency map for the current formalization: protocol
    objects and completeness are represented in Lean, the paper front doors are wired to those
    objects, and the remaining unconditional soundness work is explicitly named.  The checker is
    still a single-domain model: its folds are identity folds of the current code, so paper-STIR's
    full degree-reduction behavior is not yet represented as an unconditional theorem.  The
    front doors above are therefore conditional on the stated bridge, correlated-agreement, and
    complexity legs.
\end{remark}
