...
### [FALSE AS STATED — repair identified] PreTensorCombineJointProximityResidual
- loc: `ArkLib/ProofSystem/Binius/BinaryBasefold/Soundness/Incremental.lean:1659` (was :1120 at triage)
- **Audit wave 2 (2026-06-10) REFUTES the prior PROVABLE(M) classification.** The hypothesis `fiberwiseClose` (Code.lean:241) is degenerate — body is `2·Δ₀(f, C^(i)) < d_i` (SOURCE-level Hamming UDR; `steps`/`destIdx` phantom; `fiberwiseDistance` is a `:= 0` stub) — while the conclusion claims interleaved proximity at the DESTINATION UDR; `UDR(C^(i)) ≈ 2^steps·UDR(C^(dest))` makes it too weak. Counterexample (hand-verified): ℓ=1, 𝓡=1, r=3, ϑ=1, L=F₈, f a single-point error: `2·1 < 3 = d_0` holds but `fold(f,1)` is non-constant on S^(1), so the stack is outside interleaved `UDR(C^(1)) = 0`. The prior entry's remaining step "Δ₀(⋈|pTC f, ⋈|pTC g) ≤ Δ₀(f,g)" only yields proximity at UDR(C^(i)), not UDR(C^(dest)).
- REPAIR (= DP24 Def 4.20 honest alignment): `fiberwiseDistancePerFiber f C := min_{g∈C} |fiberwiseDisagreementSetPerFiber f g|`; redefine `fiberwiseClose i steps f := 2·fiberwiseDistancePerFiber f C^(i) < d_{i+steps}` (destination threshold); re-prove `UDRClose_of_fiberwiseClose` (`2Δ₀ ≤ 2^steps(d_dest−1) = d_i−1 < d_i`, exact); repaired close branch = deterministic counting (column-locality via `iterated_fold_eq_matrix_form` + `Finset.card_le_card`), no proximity gap. Case 1 (incremental + block) survives — it consumes the hypothesis only through `UDRClose_of_fiberwiseClose`. Full plan: `docs/kb/audits/issue-317-327-binius-residual-hypothesis-ledger-2026-06-10.md`.
- papers: DP24 2024/504 Lem 4.22 (NOTE: paper's 4.22 is the FAR branch; this close-branch dual is ArkLib-specific, never stated in the paper)
...
- CORRECTION (audit wave 2, 2026-06-10): "far→interleaved-distance = Lift.lean lemma proven" is a misreport — `lemma_4_21_interleaved_word_UDR_far` (Lift.lean:346) is a definitional contrapositive taking the distance lower bound as a HYPOTHESIS; nothing in the tree derives it from `¬fiberwiseClose` (rg-verified: `fiberwiseClose` never appears in Lift.lean). Real missing Step-1 brick: `¬fiberwiseClose f → UDR(C_dest) < Δ₀(⋈|pTC f, C_dest^⋈)` via per-fiber injectivity (M_y invertible) + interleaved-code dimension-count bijection + exactly-tight Nat arithmetic (`⌈Δ₀/2^steps⌉ ≥ UDR_dest + 1` from `Δ₀ ≥ ⌈d_i/2⌉`). All three simplify under the honest `fiberwiseClose` repair (see PreTensorCombineJointProximityResidual entry).
