...
/-- Forward decomposition: an appended-run support point yields component `R₁.run` and `R₂.run`
support points (the verifier seam split via `Verifier.append_run`, prover via `append_run_msg`). -/
theorem mem_support_append_run_decompose
    ...
    ∃ (tr1 : FullTranscript pSpec₁) (s2 : Stmt₂) (w2 : Wit₂) (sv2 : Stmt₂)
      ...
      some ((tr1, s2, w2), sv2) ∈ support ((R₁.run stmtIn witIn).run) ∧
      some ((tr2, x.1.2.1, x.1.2.2), x.2) ∈ support ((R₂.run s2 w2).run) := by
  ...
  · simp only [Option.elim, OptionT.run_bind, Option.elimM, bind_assoc, liftM_bind,
    ...
    · simp at hx
    · revert hp1 hp2; extract_goal; sorry
