...
`coeff t (Q.coeff i · γ₁^i) − coeff t (Q.coeff i · γ₂^i)`. Expanding each `coeff t (Q.coeff i · γ^i)`
over `antidiagonal t` by `coeff_mul` and subtracting, every antidiagonal pair `(a,b)` with `b < t`
cancels (its `γ^i` factor agrees by LEMMA A), leaving only the corner `(0, t)`:
