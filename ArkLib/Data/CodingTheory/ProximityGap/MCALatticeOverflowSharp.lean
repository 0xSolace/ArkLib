...
So the discrete MCA threshold is *determined* the moment one proves `ε_mca(C, j/n) ≤ ε*`
at the critical index — exactly mirroring the list-decoding reduction. The MCA lower side
currently reaches only the unique-decoding radius (`rs_mcaLowerWitness_udr`); pushing it to
Johnson is the BCIKS20 bivariate line-decoding argument, and to capacity is the open prize.
Axiom-clean
...
variable {F ι : Type} [Field F] [Fintype F] [Fintype ι] [Nonempty ι]
...
theorem mcaLatticeThreshold_lt_of_overflow (C : Set (ι → F)) {j : ℕ} {ε_star : ℝ≥0}
  ...
  have hrad :
      ((j : ℝ≥0) / (Fintype.card ι : ℝ≥0))
        ≤ ((t : ℝ≥0) / (Fintype.card ι : ℝ≥0)) := by
    gcongr
...
theorem mcaLatticeThreshold_eq_of_mem_of_overflow_succ (C : Set (ι → F)) {j : ℕ}
    {ε_star : ℝ≥0}
