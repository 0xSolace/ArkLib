...
* `weil_recovers_root_count_not_better` — **the honest no-go / cartography brick.** Even with the
  *full* Weil control (each nonzero subgroup character sum bounded by `B`, the `√q`-scale bound),
  ...
  arbitrary `(k−1)`-subset of the subgroup (the `gPoly` vanisher of `SubgroupSpectrumNoImprovement`),
  so the identity `q·agreement = n + R` reproduces `agreement = k − 1` **exactly**. The character
  sum carries no information beyond the root count: the `√q` Weil term is precisely the Johnson-scale
  fluctuation and does not enter the open interval `(1 − √ρ, 1 − ρ)`.
...
`R`. The Weil bound supplies `|∑_i ψ(g i)| ≤ (k−1)√q`, an error of `√q`-scale per character — exactly
the second-moment / Johnson fluctuation. We prove the matching lower realization: a degree-`<k`
        ...
        have hroot : ψ (g i) ^ Fintype.card F = 1 := by
          ...
          have hzero : (Fintype.card F) • (g i) = 0 := by
            rw [← Nat.cast_smul_eq_nsmul (R := F) (Fintype.card F) (g i),
              CharP.cast_card_eq_zero F, zero_smul]
...
`SubgroupSpectrumNoImprovement` evaluates to a word whose agreement with the zero word is **exactly**
`A`, i.e. the zero count is exactly `k − 1`. Hence the character-sum identity
...
example : IsPrimitiveRoot (-1 : ZMod 5) 2 := IsPrimitiveRoot.neg_one 0 (by decide)
