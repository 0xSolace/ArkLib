...
open Code Affine ReedSolomon Domain
open Polynomial
...
lemma foldWordAux_of_k_2
  ...
  have h :
    ...
    {j, j'} := by
    have h := CosetFftDomainClass.square_roots_explicit
    ...
    have := @CosetFftDomainClass.mem_subdomain_0_iff_mem (ω := domain)
    aesop
  rw [h]
  have hcard : Finset.card {j, j'} = 2 := by
    ...
    conv_rhs =>
      rw [←pow_one 2,
          ←CosetFftDomainClass.card_roots (ω := domain) (i := 0)
              (x := (CosetFftDomain.subdomain domain 1) i)
              (by omega) (by simp)]
...
private lemma roots_in_domain_card_eq_if_x_in_domain
  ...
  have h := CosetFftDomainClass.card_roots (ω := domain)
          (j := k) (i := 0) (x := x)
          (by simp [hk])
          (by aesop (add simp [CosetFftDomainClass.mem_subdomain_of_eq_vals]))
  conv_rhs =>
    rw [←h]
  exact Finset.card_bij
    (fun x _ ↦ domain x)
...
theorem folding_preserves_distance
  ...
  (hStrictCoeff : StrictCoeffPolysResidual
    (k := 2 ^ k - 1) (deg := d / (2 ^ k))
    (domain := (domain.subdomain k : Fin (2 ^ (n - k)) ↪ F)) (δ := δ))
  (δ_gt_0 : 0 < δ)
  (δ_lt : δ < min (δᵣ(f, ReedSolomon.code (domain : Fin (2 ^ n) ↪ F) d))
    ...
    have correlated_agreement :=
      correlatedAgreement_affine_curves_of_strict_coeff_polys
        (k := 2 ^ k - 1) (deg := d / (2 ^ k))
        (domain := (domain.subdomain k : Fin (2 ^ (n - k)) ↪ F))
        (δ := δ) bound_tighter
    ...
    (u : WordStack F (Fin ((2 ^ k - 1) + 1)) (Fin (2 ^ (n - k)))),
      Pr_{
        let z ← $ᵖ F}[δᵣ(∑ t : Fin ((2 ^ k - 1) + 1), (z ^ (t : ℕ)) • u t,
          ReedSolomon.code
            (domain.subdomain k : Fin (2 ^ (n - k)) ↪ F) (d / (2 ^ k))) ≤ δ] >
          (((2 ^ k - 1) : ℕ) : ENNReal) *
            (ProximityGap.errorBound δ (d / (2 ^ k))
              (domain.subdomain k : Fin (2 ^ (n - k)) ↪ F) : ENNReal) →
      (1 -
          (LinearCode.rate
            (ReedSolomon.code
              (domain.subdomain k : Fin (2 ^ (n - k)) ↪ F) (d / (2 ^ k))) :
            ℝ≥0)) / 2 < δ →
      δ < 1 - ReedSolomon.sqrtRate (d / (2 ^ k))
        (domain.subdomain k : Fin (2 ^ (n - k)) ↪ F) →
