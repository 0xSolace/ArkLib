...
`rs_mca_uptoCapacity_false_of_smallField` (`ε_mca(RS, 1−(k+1)/n) > ε*`). Combined with
the unique-decoding lower witness `rs_mcaLowerWitness_udr` (`δ* ≳ (1−ρ)/3`), this
two-sided-traps the *concrete* MCA threshold `(1−ρ)/3 ≲ δ*_mca ≤ 1 − (k+1)/n` — the MCA
analog of the list-decoding bracket. Axiom-clean (`[propext, Classical.choice, Quot.sound]`).
...
/-- **The concrete MCA threshold is below near-capacity.** For `RS[F, domain, k]` with
`1 ≤ k ≤ n` over a field with `|F| < (n−k)·2^128`, the faithful MCA threshold satisfies
`mcaDeltaStar(RS, 2^{-128}) ≤ 1 − (k+1)/n` — so the Grand MCA threshold does not reach
capacity. -/
theorem rs_mcaDeltaStar_le_capacity {F : Type} [Field F] [Fintype F]
  ...
  classical
  exact mcaDeltaStar_le_of_bad (F := F) (A := F) _ _
