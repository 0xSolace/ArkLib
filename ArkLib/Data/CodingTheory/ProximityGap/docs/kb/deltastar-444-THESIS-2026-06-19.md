...
**The single best next step (shovel-ready) — refined.** The strong form `W_3=0` is now **refuted** (exact: `W_3(32,p=2089889)=11520≠0`, `W_3(64,p=16778497)=46080≠0`; bad primes are dense just above `n^4`). But di Benedetto needs only the *exponent* `t_3=3`, i.e. `T_3=O(n^3)` — and the char-0 part is proven (`15n^3−45n^2+40n ≤ 15n^3`). So the `0.9583` exponent's conditionality is **reduced** to the weaker, plausible all-`n` excess bound **`W_3 = O(n)`** (machine-verified at `n=32,64` over the first 200 prize primes), the same finite-bad-prime/cyclotomic-norm structure as the discharged `W_4` onset work. Proving `W_3=O(n)` for all `n=2^μ` is the refined shovel-ready gate that makes `0.9583` unconditional at `β=4`. It does **not** reach `1/2` (the energy lever saturates at `0.9444`), but converts the best conditional partial into an unconditional one.
