...
**The single best next step (shovel-ready).** Prove the in-tree **No-Excess-at-`r=3`** lemma: `T_3 = O(n^3)` (equivalently `W_3 = 0` / `E_3` `p`-invariance) for all thin `β=4` primes. This is the *one* gate that upgrades the verified-at-sampled-`n` `0.9583` exponent into an **unconditional all-`n` theorem in the prize regime** — the largest honest gain available from any of the six domains, and (unlike the core) a bounded, well-posed arithmetic statement (a depth-3 instance of the F5 depth-uniform-torsion problem), already an open node in the #407 tree. It does **not** reach `1/2` (the energy lever saturates at `0.9444`), but it would convert the best conditional partial into an unconditional one.
