...
This certifies that the GS-exposed MCA framework is *sound against the classical proximity gap* where
the latter is proven. It is **not** the prize bound: `n/q` is the proximity-gap shape, incomparable
to the prize's `poly(2^m,1/ρ)/q` shape (the prize needs the GS list-size form, valid only beyond the
proximity-gap regime up to capacity — the open ABF26 core). Tracking: Issue #141.
-/
...
theorem epsMCAgs_restricted_le_errorBound_udr
    ...
    (hδ : δ ≤ Code.relativeUniqueDecodingRadius (ReedSolomon.code α deg)) :
  ...
  le_trans
    ...
    (ProximityToRS.ahiv17_epsCA_bound_uniqueDecodingRegime hδ)
