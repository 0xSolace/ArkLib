...
`RestrictedFaaDiBrunoMatchAt` only as an explicit hypothesis (using just the proven `ζ ≠ 0`), exactly
as the in-tree P2 consequence theorems do — none assumes the STEP-8 core.
...
`(t+1)` coefficient of `βHenselAssembled` is `−restrictedFaaDiBrunoSum t / ζ`. #139 analogue of
