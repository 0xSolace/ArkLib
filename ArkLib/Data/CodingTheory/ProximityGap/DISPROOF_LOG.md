...
### O91 — the Johnson-interleaved splice LANDS: a fourth unconditional ε_mca upper window, δ < (1−√(e/n))/2, strictly wider than O84's d/(3n) below rate 1/4

O89 named the governing quantity (badCount is coupled to Λ₂(2δ)) and O85 left the conversion hypothesis-shaped ("any future interleaved list bound converts with zero plumbing"); the missing producer was a Λ₂ bound beyond unique decoding. This pass supplies it: the in-tree Johnson second-moment bound (JohnsonListBound.lean) is alphabet-GENERIC (F : Type*, DecidableEq only), so it instantiates over the pair alphabet F×F directly — no pair-alphabet re-proof needed. Packing the stack (f₁,f₂) and the codeword pairs into words ι → F×F (injective), O78's distance transfer (a differing pair differs in some row; joint agreement ⊆ that row's agreement ≤ e) makes the interleaved list a Johnson list, giving Λ₂(a) ≤ n²/(a² − n·e) whenever n·e < a².

**Bricks (`EpsMCAInterleavedJohnson.lean`, exit 0, axiom-clean ×9, 0 sorry):**
* `interleavedList_card_le_johnson` — the pair-alphabet Johnson cap on the O74/O85 interleavedList surface, no linearity or window hypothesis.
* `epsMCA_le_interleavedJohnson` — **the splice**: composed through O85's `epsMCA_le_of_interleavedList_card_le` verbatim (one application, zero plumbing — the O85 claim cashed), ε_mca(C,δ) ≤ (1 + (n−a)·(n²/(a²−n·e)))/|F| at a = 2⌈(1−δ)n⌉₊ − n on n·e < a², i.e. (1+2δn·L_J)/q. Unconditional: linearity + distance parameter only.
* `johnson_window_of_sqrt` + `nat_gap_of_johnson_window` + `epsMCA_le_interleavedJohnson_of_sqrt_window` — the named real-units window 2δ + √(e/n) < 1 (δ below HALF the Johnson radius; ℝ≥0 truncation handles 2δ ≥ 1 free).
* `nat_gap_of_ud_window` — window containment: n+e < 2a ⟹ n·e < a² (AM–GM), so the O78 d/(4n) window is contained outright and the Johnson-only band is strictly new unconditional territory.
* `johnson_window_wider_of_low_rate` / `johnson_window_crossover` / `johnson_window_narrower_of_high_rate` — the O84 comparison as theorems: (1−ρ)/3 < (1−√ρ)/2 ⟺ ρ < 1/4, equality exactly at ρ = 1/4 (0.3232 vs 0.2917 at ρ=1/8; 0.375 vs 0.3125 at ρ=1/16; reversed above 1/4).

**Falsify-first probe (`probe_johnson_interleaved_splice.py`, exit 0):** exact interleaved list sizes on RS over F₅/F₇/F₁₁/F₁₇ — pair-Johnson cap 1,080 checks 0 failures; composed badCount 560 checks 0 failures, 220 full-2^n controls 0 mismatches; window arithmetic float-free on 401 exact grid points via the polynomial identity (1+2ρ)² − 9ρ = (4ρ−1)(ρ−1), 0 failures; UD⟹gap on 5,530 ℕ-points, 0 failures. **Teeth:** 216 Johnson-only-band cases with Λ₂ ≥ 2 — the UD bound is FALSE there and only the Johnson cap survives — and 120 witnesses that the gap condition is load-bearing (Λ₂ > n² below it). Demonstration instance RS(16,2)/F₁₇ at δ = 0.32: t = 11, a = 6 — outside O84's window (3(n−t) = 15 = d) and the UD window (17 ≥ 12), inside the Johnson gap (16 < 36, L_J = 12).

**Where the open core sits:** the unconditional upper-window family is now {O78 d/(4n) (UD)} ⊂ {this window (1−√(e/n))/2 (Johnson)}, incomparable with O84's d/(3n) (extraction): Johnson wins below rate 1/4, extraction above, dovetail exact at ρ = 1/4. For RS at ρ = 1/8 the unconditional floor moves 0.2917 → 0.3232 (asymptotic; finite-n even wider since e/n = (k−1)/n < ρ). The bound's numerator degrades from O(δn)/q to O(δn·L_J)/q — the price of the list. Unchanged: the gap up to δ* where the O68 lower bound C(s,r)/q lives, O74's factor-free refinement, and `InteriorCountingBound` (O89) — though note the Johnson cap now bounds the O89 coupling quantity Λ₂ explicitly inside its gap.
