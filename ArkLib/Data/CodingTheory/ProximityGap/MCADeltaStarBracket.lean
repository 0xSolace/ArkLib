...
`δ ≤ mcaDeltaStar(RS, 2^{-128}) ≤ 1 − (k+1)/n`. -/
theorem rs_mcaDeltaStar_bracket {F : Type} [Field F] [Fintype F]
    {n : ℕ} [NeZero n] (domain : Fin n ↪ F)
    ...
    (hFlo :
      2 * (Fintype.card (Fin n) - ⌈(1 - δ) * (Fintype.card (Fin n) : ℝ≥0)⌉₊)
        * 2 ^ 128
      ≤ Fintype.card F)
  ...
  have hgood : epsMCA (F := F) (A := F)
    ...
    rw [show ((2 * (Fintype.card (Fin n) - t) : ℕ) : ℝ≥0∞) /
          (Fintype.card F : ℝ≥0∞) =
        ENNReal.ofReal
          (((2 * (Fintype.card (Fin n) - t) : ℕ) : ℝ) / (Fintype.card F : ℝ)) from by
      rw [ENNReal.ofReal_div_of_pos hcardpos, ENNReal.ofReal_natCast, ENNReal.ofReal_natCast]]
    apply ENNReal.ofReal_le_ofReal
    rw [div_le_iff₀ hcardpos,
      show (1 : ℝ) / 2 ^ 128 * (Fintype.card F : ℝ) =
          (Fintype.card F : ℝ) / 2 ^ 128 from by ring,
