...
theorem rs_listDecoding_unconditional_refute
  ...
  calc
    (ε_star : ENNReal) * (Fintype.card F : ENNReal)
      = ENNReal.ofReal (1 / 2^128) * ENNReal.ofReal 512 := by
        congr 1
        · change (↑(1 / 2^128 : ℝ≥0) : ENNReal) = ENNReal.ofReal (1 / 2^128)
          rw [ENNReal.ofReal_coe_nnreal]
        · rw [hF]
          exact (ENNReal.ofReal_natCast 512).symm
    _ = ENNReal.ofReal (((1:ℝ)/2^128) * 512) := by
        rw [← ENNReal.ofReal_mul (by positivity)]
        congr 1
        norm_num
    _ < ENNReal.ofReal ((:ℝ) / (512:ℝ)^121) := h_lift
    _ = ENNReal.ofReal (↑(hammingBallVolume 512 (120 / 128) 128) / (512 : ℝ) ^ (128 - 7 : ℕ)) := by
        rw [h_vol]
        congr 2
        norm_num
    _ = ENNReal.ofReal (↑(hammingBallVolume (Fintype.card F) δ (Fintype.card (Fin 128))) / ↑(Fintype.card F) ^ (Fintype.card (Fin 128) - 7 : ℕ)) := by
        rw [hF]
        rfl
