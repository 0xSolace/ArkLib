...
Johnson already saturates. The incidence count is a pure subset count with no field structure, so it
holds for the Johnson-extremal "`k−1` freely-placed agreement positions" configuration verbatim, and
therefore cannot separate the smooth (multiplicative-subgroup) domain from a generic one. We make this
quantitative:

* `incidence_collapses_to_ceiling` — the incidence bound is driven **only** by the pairwise ceiling
  `agree(c,c') ≤ k−1`, with no use of the smoothness of the domain; the same proof applies to ANY
  family of words with that pairwise-agreement ceiling, RS or not (`abstract_incidence_bound`).
* `incidence_weaker_than_johnson_at_capacity_endpoint` — a concrete satisfiable instance
  (`n = 7, k = 2, a = 2`, the capacity-endpoint agreement) where the incidence bound gives only
  `|L| ≤ 21` while it is *vacuous as an improvement*: the bound is a subset count, not a
  field-sensitive count, so it inherits the Johnson ceiling and adds nothing past it.

The slice-rank / CLP polynomial method therefore reduces — exactly as the round-1 angles did — to the
**same** "`≤ k−1` agreement positions" wall: any improvement past Johnson must come from a
*field-sensitive and domain-sensitive* count of how the `k`-subsets actually arrange in the
multiplicative subgroup — precisely the open super-polynomial smooth-domain subset/incidence count. The
polynomial method gives the clean rank obstruction (verified here) but no new separation.
