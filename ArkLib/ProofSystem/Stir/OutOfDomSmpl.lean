...
/-- Counting a coordinatewise event: tuples satisfying `Q` in every coordinate form the
`piFinset` of the per-coordinate solution set, so their count is `(#Q)^s`. -/
...
/-- Distinct codewords decode to distinct polynomials. -/
...
/-- The agreement set of two distinct codewords' polynomials in any subtype of `F` has at most
`degree - 1` elements. -/
