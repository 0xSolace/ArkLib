...
theorem fs116_coupled_final
  ...
  -- BUILD-VERIFIED to here on Lean v4.30.0. Remaining: collapse the empty FS-NI challenge oracle
  -- on the `simulateQ (srImpl + srChallengeQueryImpl' + liftTarget challengeQueryImpl)
  -- ((monadLift X).run)` leaves (the FS-NI analog of the private
  -- `Component/NoInteraction.simulateQ_addLift_liftM_oracleComp_optionT_run_empty`, which is
  -- hardcoded to the `!p[]` spec) — `pSpec.ChallengeIdx` is `IsEmpty` for `ProverFirst`, so the
  -- oracle is never queried — then a `probEvent` congruence vs the SR game (the `∧ stmtIn ∉ langIn`
  -- conjunct discharged by `hstmtIn`, discarded `ctxOut` marginalized via the replayed `output`).
  sorry
