...
theorem mem_support_run_of_prover_verifier
  ...
  · simp only [Option.elim_some, mem_support_bind_iff]
    ...
    · simp only [support_liftM, support_map, Set.mem_image, Option.some.injEq, OptionT.run,
        liftM, monadLift, MonadLift.monadLift, OptionT.lift, OptionT.mk]
      first | exact hV | exact ⟨some vout, hV, rfl⟩ | simpa using hV
    · simp only [Option.elim_some, Option.getM_some, OptionT.run_pure, bind_pure_comp, map_pure,
        OptionT.run_bind, Option.elimM, bind_assoc, support_pure, Set.mem_singleton_iff,
        mem_support_bind_iff, support_bind, Set.mem_iUnion]
end Reduction
