...
/-- Oracle knowledge soundness implies oracle soundness whenever accepted
outputs admit a transcript-indexed witness selector that certifies the output
relation, and invalid inputs admit no witness in the input relation. -/
theorem knowledgeSoundness_implies_soundness
    ...
    (acceptWitness :
      ∀ (shared : SharedIn) (tr : Spec.Transcript (Context shared)),
        WitnessOut shared tr)
    (hLangOut :
      ∀ shared inputImpl tr stmtOut,
        langOut shared inputImpl tr stmtOut (verifier.simulate shared tr) →
          relOut shared inputImpl tr stmtOut (verifier.simulate shared tr)
            (acceptWitness shared tr)) :
    soundness verifier langIn langOut ε := by
  rcases hKS with ⟨extractor, hKS⟩
  intro shared stmt inputImpl OutputP prover ιₐ accSpec accImpl hs
  let proverKS :
  ...
  have hrun :
      OracleVerifier.run verifier shared stmt inputImpl proverKS accSpec accImpl =
        (fun z => ⟨z.1, acceptWitness shared z.1, z.2.2⟩) <$>
          OracleVerifier.run verifier shared stmt inputImpl prover accSpec accImpl := by
    unfold OracleVerifier.run
    rw [OracleDecoration.runWithOracleCounterpart_mapOutputWithRoles
      (inputImpl := inputImpl)
      (spec := Context shared)
      (roles := Roles shared)
      (od := oracleDeco shared)
      (accSpec := accSpec)
      (accImpl := accImpl)
      (fP := fun tr (_ : OutputP tr) => acceptWitness shared tr)
      prover
      (verifier shared accSpec stmt)]
    simp [Functor.map_map]
  let badFromAccept :
      ((tr : Spec.Transcript (Context shared)) × OutputP tr ×
        (StatementOut shared tr ×
          OutputImpl (Context := Context) (Roles := Roles) (oracleDeco := oracleDeco)
            OStatementIn OStatementOut shared tr)) → Prop :=
    fun z =>
      relOut shared inputImpl z.1 z.2.2.1 (verifier.simulate shared z.1)
          (acceptWitness shared z.1) ∧
        ¬ relIn shared stmt inputImpl
          (extractor shared stmt inputImpl z.1 z.2.2.1
            (verifier.simulate shared z.1)
            (acceptWitness shared z.1))
  have hKS' :
      Pr[badFromAccept | OracleVerifier.run verifier shared stmt inputImpl prover accSpec accImpl] ≤
        ε := by
    simpa [badFromAccept, hrun, proverKS, probEvent_map] using
      hKS shared stmt inputImpl proverKS accSpec accImpl
  have hmono :
      Pr[fun z => Accepts verifier langOut shared inputImpl z.1 z.2.2.1
          | OracleVerifier.run verifier shared stmt inputImpl prover accSpec accImpl] ≤
        Pr[badFromAccept
          | OracleVerifier.run verifier shared stmt inputImpl prover accSpec accImpl] := by
    apply probEvent_mono
    intro z _ hz
    refine ⟨?_, ?_⟩
    · exact hLangOut shared inputImpl z.1 z.2.2.1 hz
