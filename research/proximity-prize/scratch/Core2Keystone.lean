...
SCRATCH FILE — Core 2: connect the abstract per-round proximity gap
(`Issue24FRISTIR.PerRoundProximityGap` / `Issue113WHIR.WhirRbrKeystone`) to the
genuine BCIKS20 keystone `ProximityGap.correlatedAgreement_affine_curves`.

  *** STATUS: SCRATCH ONLY — not part of the build's module list. Compiles with
      `lake env lean`. 0 sorries / admits / axioms / native_decide as proof steps.
      Every genuine open gap remains a NAMED Prop hypothesis (the §5/§6.2 residuals
      `StrictCoeffPolysResidual` / `BoundaryProbabilityResidual`, carried verbatim
      from the keystone, plus the explicitly-named sumcheck/folding bridges). ***

  WHAT THE PER-ROUND QUANTITY *IS* (the central identification):

    The per-round proximity-gap quantity in WHIR/STIR soundness — "the probability that
    a random-linear-combined (folded) word is close to the round's RS code while the
    underlying word stack is NOT jointly close" — is precisely the numeric
    correlated-agreement error `ProximityGap.epsCA_curves Cᵢ kᵢ δᵢ δᵢ` of the round's
    Reed–Solomon code `Cᵢ = ReedSolomon.code domainᵢ degᵢ`, and (via ABF26 Fact 4.5,
    proven in-tree) the proximity-gap error `ProximityGap.epsPG Cᵢ δᵢ` is bounded by it.

    The BCIKS20 keystone `correlatedAgreement_affine_curves` outputs exactly the predicate
    `δ_ε_correlatedAgreementCurves Cᵢ δᵢ (errorBound δᵢ degᵢ domainᵢ)`, which the in-tree
    numeric bridge `δ_ε_correlatedAgreementCurves_iff_epsCA_curves_le` turns into the
    sharp numeric bound
        `epsCA_curves Cᵢ kᵢ δᵢ δᵢ ≤ kᵢ * errorBound δᵢ degᵢ domainᵢ`.
    So the round's accounting error `e i = ProxGapBound i = errorBound δᵢ degᵢ domainᵢ`
    is *exactly* the keystone's `ε`, and the keystone is *exactly* the statement that the
    per-round CA error stays below `kᵢ · ε` — i.e. the per-round proximity gap.

  DELIVERABLES (all PROVEN below, names + hypotheses recorded in the §SUMMARY):
    §1  `keystone_curves_bound`           keystone ⟹ `epsCA_curves ≤ k·errorBound`  (numeric, sharp)
    §1  `keystone_epsPG_bound`            keystone ⟹ `epsPG ≤ k·errorBound`         (Fact 4.5 bridge)
    §2  `perRoundProximityGap_of_correlatedAgreement`
                                          per-round keystone instances ⟹ Core-2's
                                          `PerRoundProximityGap` predicate + the genuine
                                          numeric CA bound for every round.
    §3  `whirRbrKeystone_of_correlatedAgreement`
                                          per-round keystone instances + budget accounting
                                          ⟹ `WhirRbrKeystone` (the `SoundOk(epsRbr)` clause).
    §4  named sumcheck/folding bridge (`FoldedStackOfRound`,
        `RoundProxGapBoundedByKeystone`) — the honest residual when the round's reduction
        is correlated-agreement PLUS a sumcheck round.
-/
