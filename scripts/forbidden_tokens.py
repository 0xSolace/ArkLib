...
(non-comment) code contains:
  ...
  - a vacuous declaration named like a residual and typed as `True`.
...
def is_residual_like(name: str, allowlist: set[str]) -> bool:
    ...
    return is_allowlisted(name, allowlist) or final.endswith("_residual")
...
def main() -> int:
    ...
    for path in files:
        ...
        for decl_index, dm in enumerate(decls):
            ...
            if result_type == "True":
                line = text.count("\n", 0, dm.start()) + 1
                failures.append(
                    f"{path}:{line}: residual-like name {name} is declared as "
    ...
    print(
        ...
        f"undocumented custom axiom / vacuous residual theorem; "
