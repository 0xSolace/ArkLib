#!/usr/bin/env python3
"""ArkLib declaration extractor — catalog every Lean `def`/`theorem`/`lemma`/etc.

Usage::

    python3 scripts/kb/extract_declarations.py \
        --root ArkLib/Data/CodingTheory ArkLib/Data/Probability ArkLib/OracleReduction \
        --out  docs/kb/_generated/declarations.json
...
A companion analysis tool (`find_dedup_candidates.py`) consumes the
JSON to group same-shortname declarations across namespaces and flag
potential duplicates.

Parsing strategy: pure regex over source text. Lean's elaborator would
be more accurate but introduces a heavy dependency (lake env). For
dedup-spotting purposes the regex is enough: it catches the canonical
declaration kinds and tracks the current `namespace`/`end` stack to
attach fully-qualified names.

Caveats:

* Multi-line declarations are recognised by their opening line only;
  the signature snippet is truncated to the first line after the name.
* Anonymous instances (``instance : Foo where ...``) are recorded under
  the synthetic name ``_anon_instance_<lineno>``.
* `where`-clauses, `theorem ... :=`, `by`-blocks etc. are not parsed
  beyond what's needed to attribute the declaration to a namespace.
"""
...
def main():
    ...
    p.add_argument(
        ...
        help="One or more directories under ArkLib/ to scan.",
    )
    p.add_argument(
        "--out", required=True,
    ...
    payload = collect(roots)
    out = REPO_ROOT / args.out
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(payload, indent=2, ensure_ascii=False))
    total = sum(s["declarations"] for s in payload["stats"].values())
    print(f"Wrote {out.relative_to(REPO_ROOT)} — {total} declarations across "
          f"{sum(s['files'] for s in payload['stats'].values())} files.")
