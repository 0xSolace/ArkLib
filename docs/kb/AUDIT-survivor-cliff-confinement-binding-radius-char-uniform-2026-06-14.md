...
METHODOLOGICAL NOTE: a naive pass showed char-p=0 vs char-0=12 at n=24 a=10 — a J-RANGE ARTIFACT
(fast pass swept only j≤k+3; the worst direction at a=10 is j=k+4). With matched j-ranges the
discrepancy vanishes (char-p=char-0=12). Any future char-p≠char-0 claim must first rule out a
j-range / dedup mismatch before being read as a refutation.

VERDICT: cliff confinement (binding-radius char-uniformity) HOLDS at every tested (n≤24,
ρ∈{1/8,1/4,1/2}) in the prize-faithful regime (proper subgroup, q≫n³). With the swarm's n=32
(ρ∈{1/4,1/8}) this is consistent multi-method evidence. The sole open question is ASYMPTOTIC: does the
char-faithfulness threshold at δ* stay poly(n³) as n→∞ (n=64 = C(64,9) solves is the compute wall).
Probes: /tmp/large_n_confirm.py, /tmp/unified_compare.py (unified barycentric char-0/char-p).
